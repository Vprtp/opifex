...
bitrate:str = "4000k"
codec:str = "libx264"
sectionLenght:float = 30 #when saving, renders multiple video of this lenght (in seconds) to avoid too much ram usage, then combines them with ffmpeg and removes the temporary files
#res:int = 720 #horizontal size, in pixels (the video is vertical in a 9:16 ratio)
...
backgroundVideoSourcePool:list[str] = [p+"source/video/background-gta01.mp4", p+"source/video/background-gta02.mp4", p+"source/video/background-mc01.mp4", p+"source/video/background-mc02.mp4", p+"source/video/background-msc01.mp4", p+"source/video/background-msc02.mp4"]
dingSound:str = p+"source/audio/ding.wav"
htmlTemplate:str = p+"source/img/reddit.html"
...
class FFmpeg:
    ...
    def overlayImage(videoPath:str, imagePath:str, endPath:str, duration:tuple[float]=(0,0.1), position:tuple[int,int]=(0,0), scale:float=1, center=False, isTemp=True): #duration=(begin time, end time) | position=(x,y) | scale changes the image size keeping it in proportion | if center=True the image will be centered in the video | isTemp=True will add the path to tempFiles, deleting it when clearTemp is called
        ...
        videoDuration:float = FFmpeg.getLenght(videoPath)
...
def getBackgroundVideo(audioPath:str,sourcePool:list[str]) -> str:
    ...
    videoLenght:float = FFmpeg.getLenght(videoPath)
...
def computeVideo(endPath:str, title:str, subtitlesPath:str, titleAudioPath:str, textAudioPath:str, name:str=accountName, videoPool:list[str]=backgroundVideoSourcePool, ding:str=dingSound, sTheme:Theme=subtitlesTheme, html:str=htmlTemplate, verbose:bool=True) -> None:
    ...
    titleLenght:float = FFmpeg.getLenght(titleAudioPath)
    dingLenght:float = FFmpeg.getLenght(ding)
    textLenght:float = FFmpeg.getLenght(textAudioPath)
    finalAudio:str = "/home/valerio/Scrivania/content-farm/temp/audio.wav"
